...
bool allocate_memory(PCB* current) {
    for(int i = 5; i >= 0; i--) { //Start from smallest partition (best-fit)
...
std::tuple<std::vector<std::string>, std::vector<int>, std::vector<external_file>>parse_args(int argc, char** argv) {
    ...
    while(std::getline(input_file, file_content)) {
        ...
        entry.program_name.erase(0, entry.program_name.find_first_not_of(" \t\r\n"));
        entry.program_name.erase(entry.program_name.find_last_not_of(" \t\r\n") + 1);
...
std::tuple<std::string, int, std::string> parse_trace(std::string trace) {
    ...
    // Trim whitespace from activity
    activity.erase(0, activity.find_first_not_of(" \t\r\n"));
    activity.erase(activity.find_last_not_of(" \t\r\n") + 1);
...
std::string print_PCB(PCB current, std::vector<PCB> _PCB) {
    ...
    // Print each waiting process in the PCB table
...
unsigned int get_size(std::string name, std::vector<external_file> external_files) {
    ...
    return (unsigned int)-1; // Return -1 (max unsigned) if not found
}
